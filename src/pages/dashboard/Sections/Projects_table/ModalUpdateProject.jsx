...
import { atrib_tech, getProject, updateProject } from "../../../../api/api_atrib";
import {
  notifyError,
  notifySuccess,
} from "../../../../components/ToastNotifications/Notification";

export default function ModalUpdateProject({ isOpen, onClose, id}) {
    const {
        register,
        handleSubmit,
        watch,
    ...
    const [projectId, setId] = React.useState(null)
    ...
    React.useEffect(() => {
        const loadTask = async () => {
            try {
                ...
                setValue("info", project.info);
                setValue("url", project.url);
                setImage(watch('image'));
                const techno = project.technology.map((tech) => ({
                    value: tech.id,
                    label: tech.descriptions,
                }));
      ...
      const onSubmit = handleSubmit(async (data) => {
        try{
            ...
            notifySuccess("Proyecto actualizado con exito")
            onClose()
        }catch(error){
            notifyError("Error al actualizar el proyecto")
